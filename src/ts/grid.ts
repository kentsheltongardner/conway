...
export default class Grid {
    ...
    set(x: number, y: number, state: number) {
        ...
        if (this.grid[index] === state) return false
    ...
    updateOrthogonal(onRules: number, offRules: number) {
        // get neighbor counts
    ...
    updateOctilinear(onRules: number, offRules: number) {
        const size = this.size
        for (let x = 0; x < size; x++) {
            const east = x == size - 1 ? 0 : x + 1
            const west = x == 0 ? size - 1 : x - 1
